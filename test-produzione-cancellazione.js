  ...
  try {
    ...
    if (pageContent.includes('Accedi al tuo account')) {
      ...
      if (emailInput && passwordInput && loginButton) {
        ...
        if (newPageContent.includes('Dashboard') || newPageContent.includes('Analisi di Fattibilità')) {
    ...
    if (projects.length > 0) {
      ...
      if (actionMenus.length > 0) {
        ...
        const deleteButtons = await page.$$('button:contains("Elimina"), [data-testid="delete-button"]');
        ...
        if (deleteButtons.length > 0) {
          ...
          const confirmButtons = await page.$$('button:contains("Conferma"), button:contains("OK"), button:contains("Sì")');
          ...
          if (confirmButtons.length > 0) {
            ...
            const projectsAfter = await page.$$('[data-testid="project-card"], .project-card, .bg-white');
