  ...
  },
  {
    ...
    handler: async (args, context) => {
      ...
      try {
        ...
        const offers = Array.from(procurementService.offers.values()).filter(
          offer => offer.rdoId === args.rdoId
        );
        const comparisons = Array.from(procurementService.comparisons.values()).filter(
          comp => comp.rdoId === args.rdoId
        );
        return {
          success: true,
          data: {
            ...
            respondedVendors: rdo.invitedVendors.filter(v => v.status === 'responded').length,
      ...
      try {
        ...
        if (args.activeOnly !== false) {
          filteredVendors = filteredVendors.filter(v =>
            v.documents.every(d => d.status === 'valid')
          );
        }
