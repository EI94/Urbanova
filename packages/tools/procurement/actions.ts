...
export const procurementActions: ToolActionSpec[] = [
  {
    ...
    handler: async (args, context) => {
      const procurementService = context.services.procurement as ProcurementService;
...

  {
    ...
    description:
      'Confronta automaticamente le offerte ricevute con scoring ponderato e outlier detection',
    parameters: {
      type: 'object',
      properties: {
        rdoId: {
        ...
        weights: {
          ...
          description:
            'Pesi personalizzati per il scoring (opzionale, default: price 0.7, time 0.2, quality 0.1)',
        },
      },
      required: ['rdoId'],
    },
...

  {
    ...
    handler: async (args, context) => {
      ...
      try {
        const result = await procurementService.awardRDO(
          args.rdoId,
          args.vendorId,
          args.overridePreCheck || false
        );
...

  {
    ...
    handler: async (args, context) => {
      ...
      try {
        ...
        const rdo = (procurementService as any).rdos.get(args.rdoId);
        if (!rdo) {
          throw new Error(`RDO ${args.rdoId} non trovato`);
        }

        const offers = Array.from((procurementService as any).offers.values()).filter(
          (offer: any) => offer.rdoId === args.rdoId
        );

        const comparisons = Array.from((procurementService as any).comparisons.values()).filter(
          (comp: any) => comp.rdoId === args.rdoId
        );
        ...
        return {
          ...
          data: {
            ...
            respondedVendors: rdo.invitedVendors.filter((v: any) => v.status === 'responded')
              .length,
...

  {
    ...
    handler: async (args, context) => {
      ...
      try {
        const vendors = Array.from((procurementService as any).vendors.values());

        let filteredVendors = vendors;

        if (args.category) {
          filteredVendors = vendors.filter((v: any) => v.category.includes(args.category));
        }

        if (args.activeOnly !== false) {
          filteredVendors = filteredVendors.filter((v: any) =>
            v.documents.every((d: any) => d.status === 'valid')
          );
        }

        const vendorList = filteredVendors.map((vendor: any) => ({
          id: vendor.id,
          name: vendor.name,
          email: vendor.email,
          category: vendor.category,
          rating: vendor.rating,
          documents: vendor.documents.map((d: any) => ({
            type: d.type,
            status: d.status,
            expiresAt: d.expiresAt?.toISOString(),
          })),
          status: vendor.documents.every((d: any) => d.status === 'valid') ? 'active' : 'inactive',
        }));
