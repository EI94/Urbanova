...
export class ProcurementService {
  private rdos = new Map<string, RDO>();
  ...
  async createRDO(args: {
    ...
    // Validazione input
    const validatedArgs = zRDO.parse({
      id: 'temp',
   ...
   */
  async compareOffers(rdoId: string, weights?: ScoringWeights): Promise<Comparison> {
   ...
   */
  private async performPreCheck(vendorId: string, rdoId: string): Promise<PreCheckResult> {
    const vendor = this.vendors.get(vendorId);
    if (!vendor) {
      throw new Error(`Vendor ${vendorId} non trovato`);
    }
   ...
   */
  private generateSecureToken(rdoId: string, vendorId: string, expiresAt: Date): string {
    const payload = {
      rdoId,
      vendorId,
