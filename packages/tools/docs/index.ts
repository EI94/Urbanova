...
export const docHunterManifest: ToolManifest = {
  id: 'docs',
...
export const docHunterActions: ToolActionSpec[] = [
  {
    name: 'request_doc',
...
export const docHunterTool = {
  manifest: docHunterManifest,
  actions: docHunterActions,

  async request_doc(
    ctx: any,
    args: { projectId: string; kind: 'CDU' | 'VISURA' | 'DURC' | 'PLANIMETRIA'; recipient: string }
  ) {
  ...
  async status(ctx: any, args: { projectId: string }) {
