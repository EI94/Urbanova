...
class GoogleCalendarService {
    ...
    async createEvent(request) {
        ...
        // Partecipanti
        const attendees = request.participants
    ...
    async updateEvent(eventId, updates) {
        ...
        if (updates.start && googleEvent.start)
            googleEvent.start.dateTime = updates.start;
        if (updates.end && googleEvent.end)
            googleEvent.end.dateTime = updates.end;
        if (updates.attendees)
            googleEvent.attendees = updates.attendees;
    ...
    async syncWithGoogleCalendar() {
        ...
        try {
            ...
            const googleEvents = await this.googleClient.listEvents(this.calendarConfig.calendarId, now, oneWeekFromNow);
            ...
            console.log(`🔄 Google Calendar Sync - Created: ${results.created}, Updated: ${results.updated}, Errors: ${results.errors.length}`);
        }
        catch (error) {
    ...
    async findOverlappingEvents(event) {
        ...
        return allEvents.filter(otherEvent => {
            if (otherEvent.id === event.id)
                return false;
            if (!otherEvent.start?.dateTime || !otherEvent.end?.dateTime)
                return false;
