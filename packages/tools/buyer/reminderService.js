...
Grazie! 🏠`,
    ...
    async scheduleReminder(request) {
        ...
        console.log(`⏰ Reminder Scheduled - ID: ${reminder.id}, Channel: ${request.channel}, Template: ${request.template}`);
        return reminder;
    }
    /**
     * Invia reminder
    ...
    async sendReminder(reminderId, channels = ['whatsapp', 'email']) {
        ...
        for (const channel of channels) {
            try {
                ...
                switch (channel) {
                    case 'whatsapp':
                        sent = await this.sendWhatsApp(reminder.data?.to || reminder.data?.phone || reminder.recipient, reminder.template, reminder.data || {});
                        break;
                    case 'email':
                        sent = await this.sendEmail(reminder.data?.to || reminder.data?.email || reminder.recipient, reminder.template, reminder.data || {});
                        break;
                    case 'sms':
                        sent = await this.sendSMS(reminder.data?.to || reminder.data?.phone || reminder.recipient, reminder.template, reminder.data || {});
                        break;
                }
