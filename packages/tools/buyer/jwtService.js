...
class JWTService {
    constructor() {
        ...
        this.secretKey = process.env.JWT_SECRET_KEY || 'urbanova-buyer-secret-key-2024';
    ...
    async generateUploadLink(request) {
        ...
        console.log(`🔗 Upload Link Generated - ID: ${jwtLink.id}, Buyer: ${request.buyerId}, Expires: ${expiresAt.toISOString()}`);
        return jwtLink;
    }
    /**
     * Genera link accesso
    ...
    async generateAccessLink(request) {
        ...
        this.jwtLinks.set(jwtLink.id, jwtLink);
        console.log(`🔗 Access Link Generated - ID: ${jwtLink.id}, Buyer: ${request.buyerId}, Max Uses: ${request.maxUses || 1}`);
        return jwtLink;
    }
    /**
     * Verifica token
     ...
     * Ottieni JWT link
     */
    async getJWTLink(jwtLinkId) {
        return this.jwtLinks.get(jwtLinkId) || null;
    }
    /**
     * Lista JWT link
     */
    async listJWTLinks(filters = {}) {
    ...
    generateJWT(payload) {
        ...
        // Simulazione firma
        const signature = Buffer.from(`${encodedHeader}.${encodedPayload}.${this.secretKey}`).toString('base64url');
