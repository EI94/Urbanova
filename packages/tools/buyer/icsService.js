...
class ICSService {
    ...
    generateICSContent(appointment, options) {
        ...
        const attachments = options.includeAttachments && appointment.attachments && appointment.attachments.length > 0
        ...
        attendees.forEach(attendee => {
            icsContent.push(`ATTENDEE;CUTYPE=INDIVIDUAL;ROLE=${attendee.role};PARTSTAT=NEEDS-ACTION;RSVP=TRUE;CN=${attendee.name}:mailto:${attendee.email}`);
        });
        // Aggiungi allegati
        ...
        // Aggiungi ricorrenza
        if (recurrence) {
            icsContent.push(`RRULE:FREQ=${recurrence.freq};INTERVAL=${recurrence.interval}${recurrence.until ? `;UNTIL=${recurrence.until.toISOString().replace(/[-:]/g, '').split('.')[0]}Z` : ''}`);
        }
    ...
    async updateICSFile(icsFileId, updates) {
        ...
        if (updates.downloadUrl)
            icsFile.downloadUrl = updates.downloadUrl;
        if (updates.expiresAt)
            icsFile.expiresAt = updates.expiresAt;
