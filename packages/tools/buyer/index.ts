...
import { BuyerTool } from './buyerTool';
...
export const buyerManifest: ToolManifest = {
  id: 'buyer',
 ...
 * Buyer Actions
 */
export const buyerActions: ToolActionSpec[] = [
  {
...

  {
    ...
    handler: async (params, context) => {
      const tool = new BuyerTool();
      return await tool.listAppointments(
        params.buyerId,
        params.status,
...

  {
    ...
    handler: async (params, context) => {
      const tool = new BuyerTool();
      return await tool.updatePrivacy(
        params.buyerId,
        params.retentionPolicy,
        params.dataSubjectRights
      );
    },
  },
];
