...
class PrivacyService {
    ...
    async updateBuyer(buyerId, updates) {
        ...
        // Aggiorna campi
        if (updates.name)
            buyer.name = updates.name;
        if (updates.email)
            buyer.email = updates.email;
        if (updates.phone)
            buyer.phone = updates.phone;
        if (updates.preferences)
            buyer.preferences = { ...buyer.preferences, ...updates.preferences };
        if (updates.status)
            buyer.status = updates.status;
    ...
    async deleteBuyerData(buyerId, reason) {
        ...
        this.auditLog = this.auditLog.filter(log => log.buyerId !== buyerId || log.event === 'data_deletion_requested');
        this.logAuditEvent('data_deleted', {
            buyerId,
            deletionDate: new Date(),
            reason,
        });
        console.log(`🗑️ Buyer Data Deleted - ID: ${buyerId}, Reason: ${reason || 'No reason provided'}`);
        return true;
    }
    /**
        ...
        for (const buyer of buyers) {
            try {
                ...
                if (retentionCheck.shouldDelete) {
                    const deletedSuccessfully = await this.deleteBuyerData(buyer.id, 'Automatic retention policy');
                ...
                errors.push(`Error processing buyer ${buyer.id}: ${error instanceof Error ? error.message : 'Unknown error'}`);
            }
        }
        console.log(`🧹 Retention Policy Applied - Processed: ${processed}, Deleted: ${deleted}, Errors: ${errors.length}`);
