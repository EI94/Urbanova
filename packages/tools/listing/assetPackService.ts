...
export class AssetPackService {
   ...
   */
  private async getDescriptions(listingPayload: ListingPayload) {
    return [
      {
...
Condizioni: ${listingPayload.condition}`,
   ...
   */
  private async getImages(listingPayload: ListingPayload, options?: any) {
    const images = [];
   ...
   */
  private async processImage(img: any, options?: any) {
   ...
   */
  private async getDocuments(listingPayload: ListingPayload) {
    const documents = [];
   ...
   */
  private async getPlanimetrie(projectId: string) {
    console.log(
      `🏗️ [AssetPackService] Recupero planimetrie da Design Center per progetto ${projectId}`
    );
   ...
   */
  private async getMarketingMaterials(projectId: string) {
...
#immobili #vendita #${projectId}`,
   ...
   */
  private generateMetadata(listingPayload: ListingPayload) {
    return {
   ...
   * Crea pacchetto ZIP
   */
  private async createZipPackage(projectId: string, assets: any) {
   ...
   */
  private async validateAssetPack(zipUrl: string, assets: any) {
    ...
    const missingTypes = requiredAssetTypes.filter(
      type => !assets[type] || assets[type].length === 0
    );
  ...
  generateAssetPackReport(assets: any): string {
    ...
    Object.entries(assets).forEach(([type, items]: [string, any]) => {
