...
export class FeedGeneratorService {
   ...
   */
  private generateGetrixFeed(listingPayload: ListingPayload, zipUrl: string): string {
...
<feed xmlns="http://www.getrix.com/schema/1.0">
   ...
   */
  private generateImmobiliareFeed(listingPayload: ListingPayload, zipUrl: string): string {
    const timestamp = new Date().toISOString();
...
<immobiliare_feed>
   ...
   */
  private generateGenericFeed(listingPayload: ListingPayload, zipUrl: string): string {
    const timestamp = new Date().toISOString();
...
<listing_feed>
   ...
   */
  private generateCasaFeed(listingPayload: ListingPayload, zipUrl: string): string {
    return this.generateGenericFeed(listingPayload, zipUrl);
  }

  private generateIdealistaFeed(listingPayload: ListingPayload, zipUrl: string): string {
    return this.generateGenericFeed(listingPayload, zipUrl);
  }
