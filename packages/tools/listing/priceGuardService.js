...
class PriceGuardService {
    ...
    async checkPriceGuard(listingPayload, priceGuard) {
        console.log(`🛡️ [PriceGuardService] Verifica price guard per progetto ${listingPayload.projectId}`);
        ...
        if (pricePerSqm < priceGuard.minPricePerSqm) {
            violations.push({
                ...
                differencePct: Math.round(((priceGuard.minPricePerSqm - pricePerSqm) / priceGuard.minPricePerSqm) * 100),
            });
        }
        if (pricePerSqm > priceGuard.maxPricePerSqm) {
            violations.push({
                ...
                differencePct: Math.round(((pricePerSqm - priceGuard.maxPricePerSqm) / priceGuard.maxPricePerSqm) * 100),
            });
        }
