...
exports.listingTool = exports.ListingTool = exports.listingActions = exports.listingManifest = void 0;
...
class ListingTool {
     ...
     */
    async prepare(args) {
        console.log(`🔧 [ListingTool] Preparazione annuncio per progetto ${args.projectId} su portale ${args.portal}`);
        ...
        const violations = await this.priceGuardService.checkPriceGuard(listingPayload, args.priceGuard);
        // 3. Genera asset pack (ZIP)
        const zipUrl = await this.assetPackService.generateAssetPack(args.projectId, listingPayload, args.options);
        // 4. Genera feed XML
        const feedUrl = await this.feedGeneratorService.generateFeed(args.portal, listingPayload, zipUrl);
        // 5. Calcola metadata
     ...
     */
    async push(args) {
        console.log(`🚀 [ListingTool] Pubblicazione annuncio per progetto ${args.projectId} su portale ${args.portal}`);
        ...
        if (args.feedResult.violations && args.feedResult.violations.length > 0) {
            if (!args.confirmOverride) {
                throw new Error(`Pubblicazione bloccata: violazioni price guard rilevate. Richiedi conferma esplicita.`);
            }
            console.log(`⚠️ [ListingTool] Override confermato per violazioni: ${args.overrideReason}`);
        }
        // 2. Carica feed e ZIP su GCS
        const portalUrl = await this.listingService.uploadToPortal(args.portal, args.projectId, args.feedResult);
        // 3. Log istruzioni per upload manuale (v1)
        const manualInstructions = this.generateManualInstructions(args.portal, args.projectId, args.feedResult);
