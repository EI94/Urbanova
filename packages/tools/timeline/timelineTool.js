...
class TimelineTool {
    ...
    async replanTimeline(projectId, cause, details, options) {
        try {
            ...
            console.log(`📊 [TimelineTool] Impatto analizzato: ${proposal.impact.totalDelay} giorni di ritardo`);
    ...
    async getCriticalPath(projectId) {
        try {
            ...
            const criticalTasks = timeline.wbs.tasks.filter(task => timeline.wbs.criticalPath.includes(task.id));
            return {
                exists: true,
