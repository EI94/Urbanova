...
class TimelineService {
    ...
    async updateTimeline(projectId, newWBS) {
        try {
            ...
            if (timeline) {
                ...
                console.log(`✅ [TimelineService] Timeline ${timeline.id} aggiornata alla versione ${timeline.version}`);
            }
            else {
    ...
    async getRePlanHistory(projectId) {
        try {
            ...
            const timeline = this.timelines.get(projectId);
            if (timeline) {
                console.log(`✅ [TimelineService] Storico re-plan recuperato: ${timeline.rePlanHistory.length} proposte`);
    ...
    async getActiveTriggers(projectId) {
        try {
            ...
            const timeline = this.timelines.get(projectId);
            if (timeline) {
                const activeTriggers = timeline.activeTriggers.filter(t => t.status === 'detected' || t.status === 'analyzing' || t.status === 'proposed');
    ...
    async restoreTimeline(backup) {
        try {
            console.log(`🔄 [TimelineService] Ripristino timeline da backup per progetto ${backup.projectId}`);
            if (backup.timeline) {
