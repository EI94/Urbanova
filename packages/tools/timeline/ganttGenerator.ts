import { WBS, WBSTask, RePlanProposal, RePlanPreview } from '@urbanova/types';
...
export class GanttGenerator {
   ...
   */
  async generateRePlanPreview(proposal: RePlanProposal): Promise<RePlanPreview> {
  ...
  private generateSVGContent(
    ...
    // Genera task bars
    const taskBars = this.generateTaskBars(
      tasks,
      timelineStart,
      timelineDuration,
      timelineWidth,
   ...
   */
  private generateLabels(tasks: WBSTask[], taskHeight: number, taskSpacing: number): string {
  ...
  private generateDependencies(
    ...
    dependencies.forEach(dep => {
      ...
      if (fromTask && toTask) {
        ...
        const fromX =
          ((fromTaskEnd.getTime() - timelineStart.getTime()) / timelineDuration) * timelineWidth;
        const toX =
          ((toTaskStart.getTime() - timelineStart.getTime()) / timelineDuration) * timelineWidth;
   ...
   * Genera summary dei cambiamenti
   */
  private generateChangesSummary(proposal: RePlanProposal): {
    summary: string;
    details: string[];
   ...
   * Genera summary dell'impatto
   */
  private generateImpactSummary(proposal: RePlanProposal): {
    summary: string;
    risks: string[];
    benefits: string[];
