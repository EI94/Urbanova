...
class RePlanEngine {
     ...
     */
    async detectTrigger(projectId, cause, details) {
     ...
     */
    async detectDocumentExpiryTriggers(projectId) {
        const triggers = [];
        ...
        for (const doc of expiringDocs) {
            const daysUntilExpiry = Math.ceil((doc.validUntil.getTime() - Date.now()) / (24 * 60 * 60 * 1000));
     ...
     */
    async detectSALDelayTriggers(projectId) {
        const triggers = [];
        ...
        for (const sal of delayedSALs) {
            const delayDays = Math.ceil((Date.now() - sal.expectedResponseDate.getTime()) / (24 * 60 * 60 * 1000));
            if (delayDays > 0) {
                triggers.push({
     ...
     */
    async detectProcurementDelayTriggers(projectId) {
        const triggers = [];
     ...
     */
    async detectResourceConflictTriggers(projectId) {
        const triggers = [];
        ...
        for (const conflict of resourceConflicts) {
            triggers.push({
                ...
                severity: conflict.overlapDays > 15 ? 'critical' : conflict.overlapDays > 7 ? 'high' : 'medium',
                detectedAt: new Date(),
                impact: {
     ...
     */
    analyzeTriggerType(cause, details) {
     ...
     */
    calculateTriggerSeverity(cause, details) {
     ...
     */
    async analyzeTriggerImpact(projectId, cause, details) {
     ...
     */
    async getCurrentTimeline(projectId) {
     ...
     */
    async generateProposedTimeline(trigger, currentTimeline) {
     ...
     */
    async calculateChanges(currentTimeline, proposedTimeline) {
     ...
     */
    determineConfirmationRequired(impact) {
        // Logica per determinare se serve conferma
        const requiresConfirmation = impact.totalDelay > 14 || impact.riskAssessment.overallRisk === 'high';
        return {
...
exports.RePlanEngine = RePlanEngine;
//# sourceMappingURL=rePlanEngine.js.map