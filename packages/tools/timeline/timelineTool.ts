...
import { AutoWBSEngine } from './autoWBSEngine';
import { RePlanEngine } from './rePlanEngine';
...
export class TimelineTool {
  ...
  async replanTimeline(
    ...
    try {
      ...
      let preview: RePlanPreview | undefined;
   ...
   */
  async getTimelineStatus(projectId: string): Promise<any> {
    try {
   ...
   */
  async detectTriggers(projectId: string): Promise<any> {
    try {
   ...
   */
  private async collectFacts(projectId: string): Promise<Fact[]> {
    const facts: Fact[] = [];
   ...
   */
  private async collectDocHunterFacts(projectId: string): Promise<DocHunterFact[]> {
   ...
   */
  private async collectProcurementFacts(projectId: string): Promise<ProcurementFact[]> {
   ...
   * Raccogli fatti SAL
   */
  private async collectSALFacts(projectId: string): Promise<SALFact[]> {
   ...
   */
  private async collectListingFacts(projectId: string): Promise<ListingFact[]> {
   ...
   */
  private async calculateCriticalPath(wbs: WBS): Promise<WBSTask[]> {
    // Implementazione algoritmo critical path
    const criticalTasks: WBSTask[] = [];
