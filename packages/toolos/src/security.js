...
class SecurityManager {
    ...
    checkRole(action, context) {
        ...
        if (!hasPermission) {
            context.logger.warn(`Permesso negato: ${userRole} (livello ${userLevel}) non può eseguire action che richiede ${requiredRole} (livello ${requiredLevel})`);
        }
    ...
    checkProjectAccess(context) {
        ...
        context.logger.info(`Accesso al progetto ${context.projectId} verificato per utente ${context.userId}`);
        return true;
    }
    /**
     * Verifica se l'utente può accedere al workspace
     */
    checkWorkspaceAccess(context) {
        ...
        context.logger.info(`Accesso al workspace ${context.workspaceId} verificato per utente ${context.userId}`);
        return true;
    }
    /**
