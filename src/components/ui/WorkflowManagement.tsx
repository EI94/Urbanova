...
export default function WorkflowManagement({
  ...
  const [activeTab, setActiveTab] = useState<'workflows' | 'templates' | 'approvals' | 'analytics'>('workflows');
  ...
  const handleCreateWorkflow = () => {
    try {
      ...
      setNewWorkflow({ templateId: '', name: '', description: '', priority: 'medium', context: {} });
        ...
        <div className="bg-gradient-to-r from-purple-600 to-blue-600 text-white p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-purple-100">Gestisci workflow, approvazioni e processi collaborativi</p>
            </div>
            <button
              onClick={onClose}
        ...
        <div className="border-b border-gray-200">
          <nav className="flex space-x-8 px-6">
            {[
              ...
              { id: 'approvals', name: '✅ Approvazioni', count: notifications.filter(n => n.type === 'approval_required').length },
            ...
            <div className="space-y-6">
              ...
              {workflows.length === 0 && (
                <div className="text-center py-12">
                  ...
                  <h3 className="text-lg font-medium text-gray-900 mb-2">Nessun workflow trovato</h3>
                  <p className="text-gray-500">Crea il tuo primo workflow per iniziare a collaborare!</p>
                </div>
              )}
            </div>
          )}
          ...
          {activeTab === 'approvals' && (
            <div className="space-y-6">
              ...
              {notifications.filter(n => n.type === 'approval_required').length === 0 && (
                <div className="text-center py-12">
                  <div className="text-gray-400 text-6xl mb-4">✅</div>
                  <h3 className="text-lg font-medium text-gray-900 mb-2">Nessuna approvazione richiesta</h3>
                ...
                <div className="bg-white border border-gray-200 rounded-lg p-4">
                  <div className="flex items-center">
                    ...
                    <div className="ml-3">
                      ...
                      <p className="text-2xl font-semibold text-gray-900">{metrics.approvalRate.toFixed(1)}%</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setNewWorkflow(prev => ({ ...prev, templateId: e.target.value }))}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <Badge className={getStatusColor(selectedWorkflow.status)}>
                      {getStatusIcon(selectedWorkflow.status)} {selectedWorkflow.status.replace('_', ' ')}
                    </Badge>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-900">{selectedWorkflow.createdAt.toLocaleDateString()}</p>
                  </div>
                </div>
                      ...
                      <div key={step.id} className="border border-gray-200 rounded-lg p-3">
                          ...
                          <div className="flex items-center space-x-2">
                            <Badge className={
                              ...
                              {step.status === 'completed' ? '✅' : 
                               step.status === 'in_progress' ? '⏳' : '⏸️'} {step.status}
                            </Badge>
                            {step.requiredRole && (
                              <Badge className="bg-blue-100 text-blue-800">
                                👤 {step.requiredRole}
                        ...
                        {step.status === 'in_progress' && step.requiredRole === currentUserRole && (
                          <div className="mt-3 p-3 bg-gray-50 rounded-lg">
                            <h6 className="font-medium text-gray-900 mb-2">Richiesta approvazione</h6>
                            <div className="space-y-3">
                              <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">Azione</label>
                                <select
                                  value={approvalForm.action}
                                  onChange={(e) => setApprovalForm(prev => ({ ...prev, action: e.target.value as ApprovalAction }))}
                              ...
                              <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">Commenti</label>
                                <textarea
                          ...
                          <div className="flex items-center space-x-2">
                            <span className="text-sm text-gray-600">
                              {approval.action === 'approve' ? '✅' : 
                               ...
                               approval.action === 'request_changes' ? '🔄' : '👥'} {approval.action}
                            </span>
