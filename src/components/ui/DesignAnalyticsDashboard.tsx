...
export default function DesignAnalyticsDashboard({
  templates,
  projects,
}: DesignAnalyticsDashboardProps) {
  ...
  const [timelineTrends, setTimelineTrends] = useState<
    { month: string; projects: number; avgROI: number }[]
  >([]);
  ...
  const calculateAnalytics = () => {
    ...
    setTimeout(() => {
      ...
      const templatePerformance = templates.map(template => {
        ...
        const avgROI =
          templateProjects.length > 0
            ? templateProjects.reduce((sum, p) => sum + (p.estimatedROI || 15), 0) /
      ...
      const mostPopularCategory =
        Object.entries(categories).sort(([, a], [, b]) => b - a)[0]?.[0] || 'residenziale';
            ...
            <div className="flex items-center space-x-3 mb-3">
              ...
              <div>
                ...
                <p className="text-lg font-semibold text-gray-900">
                  {metrics.averageTimeline.toFixed(1)} settimane
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="text-center p-4 bg-blue-50 border border-blue-200 rounded-lg">
            ...
            <div className="text-sm text-blue-600">
              {metrics.totalProjects > 0
                ? ((metrics.activeProjects / metrics.totalProjects) * 100).toFixed(1)
                : 0}
              %
            </div>
          </div>
