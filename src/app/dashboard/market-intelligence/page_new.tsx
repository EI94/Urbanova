...
export default function LandScrapingPage() {
  ...
  const handleSearch = async () => {
    if (!searchCriteria.location) {
      toast.error('Inserisci una localizzazione');
      return;
    }
    ...
    try {
      ...
      toast.success(`Ricerca completata! Trovati ${data.results?.length || 0} risultati`);
      ...
      <div className="space-y-6">
        ...
        <TeamCollaborationPanel
          ...
          onStartCollaborativeSession={() => {
            ...
            setShowCollaborativeSession(true);
          }}
        />

        <CollaborativeSearchSession
          isOpen={showCollaborativeSession}
          onClose={() => setShowCollaborativeSession(false)}
          searchCriteria={searchCriteria}
          onStartCommentsVoting={() => {
            setShowCollaborativeSession(false);
            setShowCommentsVoting(true);
          }}
        />

        <TeamCommentsVoting
          isOpen={showCommentsVoting}
          onClose={() => setShowCommentsVoting(false)}
