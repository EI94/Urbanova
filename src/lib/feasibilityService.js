...
exports.feasibilityService = exports.FeasibilityService = void 0;
...
class FeasibilityService {
  constructor() {
    ...
    this.COMPARISON_COLLECTION = 'feasibilityComparisons';
  }
  ...
  async createProjectWithTransaction(projectData) {
    try {
      ...
      return await (0, firestore_1.runTransaction)(firebase_1.db, async transaction => {
        const project = {
          ...projectData,
          createdAt: new Date(),
          updatedAt: new Date(),
        ...
        console.log(
          `✅ Progetto fattibilità creato con transazione: ${project.name} con ID: ${docRef.id}`
        );
        return docRef.id;
      });
    } catch (error) {
  ...
  async createProjectWithBatch(projectData) {
    try {
  ...
  async getAllProjects() {
    try {
      ...
      const projectsRef = (0, firestore_1.collection)(firebase_1.db, this.COLLECTION);
      const q = (0, firestore_1.query)(projectsRef, (0, firestore_1.orderBy)('createdAt', 'desc'));
  ...
  async getProjectsByUser(userId) {
    try {
      ...
      const q = (0, firestore_1.query)(
        projectsRef,
        (0, firestore_1.where)('createdBy', '==', userId),
  ...
  async getProjectById(id) {
    try {
      ...
      const docRef = (0, firestore_1.doc)(firebase_1.db, this.COLLECTION, id);
  ...
  async updateProject(id, updates) {
    try {
      ...
      const projectRef = (0, firestore_1.doc)(firebase_1.db, this.COLLECTION, id);
  ...
  async testConnection() {
    try {
      ...
      const testRef = (0, firestore_1.doc)(firebase_1.db, 'test', 'feasibility');
  ...
  calculateCosts(project, constructionCostMode = 'total') {
    try {
      ...
      // Calcola subtotali
      costs.land.subtotal =
        costs.land.purchasePrice + costs.land.purchaseTaxes + costs.land.intermediationFees;
      ...
      costs.construction.subtotal =
        costs.construction.excavation +
        costs.construction.structures +
        costs.construction.systems +
        costs.construction.finishes;
      // Calcola totale
      costs.total =
        costs.land.subtotal +
        costs.construction.subtotal +
        costs.externalWorks +
        costs.concessionFees +
        costs.design +
        costs.bankCharges +
        costs.exchange +
        costs.insurance;
  ...
  calculateResults(costs, revenues, targetMargin) {
    try {
    ...
    try {
      ...
      if (totalCosts > 0 && totalRevenues > 0) {
        ...
        if (margin < project.targetMargin) {
          risks.push(
            `Margine inferiore al target (${margin.toFixed(1)}% vs ${project.targetMargin}%)`
          );
  ...
  async getProjectsRanking() {
    try {
      ...
      const rankedProjects = projectsWithScore.sort(
        (a, b) => (b.feasibilityScore || 0) - (a.feasibilityScore || 0)
      );
  ...
  async recalculateAllProjects() {
    try {
      ...
      console.log(
        `✅ Ricalcolo completato: ${updatedCount}/${projects.length} progetti aggiornati`
      );
    } catch (error) {
  ...
  async compareProjects(project1Id, project2Id, userId) {
    try {
      ...
      const comparisonRef = (0, firestore_1.collection)(firebase_1.db, 'feasibilityComparisons');
      await (0, firestore_1.addDoc)(comparisonRef, comparison);
...
exports.feasibilityService = new FeasibilityService();
//# sourceMappingURL=feasibilityService.js.map
