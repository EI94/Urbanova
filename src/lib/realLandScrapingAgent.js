...
exports.realLandScrapingAgent = exports.RealLandScrapingAgent = void 0;
const advancedWebScraper_1 = require('./advancedWebScraper');
const cacheService_1 = require('./cacheService');
const realAIService_1 = require('./realAIService');
const realEmailService_1 = require('./realEmailService');
const realWebScraper_1 = require('./realWebScraper');
class RealLandScrapingAgent {
  constructor() {
    this.name = 'Real AI Land Scraper';
    this.version = '2.0';
  }
  async runAutomatedSearch(criteria, email) {
    ...
    try {
      ...
      await realWebScraper_1.realWebScraper.initialize();
      // 1. Controlla Cache per velocità
      console.log('🔍 Fase 1: Controllo Cache...');
      const cachedResult = await cacheService_1.cacheService.get(criteria);
      if (cachedResult) {
        console.log('⚡ Risultati trovati in cache!');
        await realWebScraper_1.realWebScraper.close();
        return cachedResult;
      }
      ...
      const locations = criteria.locations || [criteria.location];
      ...
      for (const loc of locations) {
        ...
        try {
          const locationLands =
            await advancedWebScraper_1.advancedWebScraper.scrapeLandsAdvanced(locationCriteria);
          ...
          if (locationLands.length === 0) {
            ...
            const traditionalLands =
              await realWebScraper_1.realWebScraper.scrapeLands(locationCriteria);
      ...
      try {
        const analysisPromises = topLands.map(async land => {
          try {
            const landAnalysis = await realAIService_1.realAIService.analyzeLand(land);
            land.aiScore = await realAIService_1.realAIService.calculateAdvancedAIScore(
              land,
              landAnalysis
            );
            return landAnalysis;
      ...
      const [marketTrends, recommendations] = await Promise.all([
        realAIService_1.realAIService.analyzeMarketTrends(criteria.location || 'Italia'),
        realAIService_1.realAIService.generateInvestmentRecommendations(filteredLands),
      ]);
      // 4. Prepara Summary
      const summary = {
      ...
      await cacheService_1.cacheService.set(criteria, result, 15 * 60 * 1000); // 15 minuti
  ...
  async sendEmailNotification(email, lands, summary, analysis) {
    ...
    await realEmailService_1.realEmailService.sendEmail(notification);
  }
  generateAdvancedEmailHTML(lands, summary, analysis) {
    ...
    const recommendationsHTML = summary.recommendations
      .map(rec => `<li style="margin-bottom: 4px; color: #374151;">${rec}</li>`)
      .join('');
    return `
      <!DOCTYPE html>
      <html>
  ...
  async saveSearchResults(lands, analysis, summary, criteria, email) {
    try {
      const searchResult = {
        email,
        criteria,
  ...
  async verifyAllServices() {
    ...
    try {
      ...
      results.email = await realEmailService_1.realEmailService.verifyEmailConfig();
    } catch (error) {
    ...
    try {
      // Verifica AI Service
      results.ai = await realAIService_1.realAIService.verifyAIConfig();
    } catch (error) {
...
exports.RealLandScrapingAgent = RealLandScrapingAgent;
// Istanza singleton
exports.realLandScrapingAgent = new RealLandScrapingAgent();
//# sourceMappingURL=realLandScrapingAgent.js.map
