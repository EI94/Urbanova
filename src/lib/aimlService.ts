...
export class AIMLService {
        ...
        features: [
          {
            ...
            preprocessing: [{ method: 'one_hot_encoding', parameters: {} }],
            statistics: {
              nullCount: 0,
              uniqueCount: 250,
              mostFrequent: 'Milano',
            ...
            validationRules: [
              ...
              {
                rule: 'in_category_list',
                parameters: { categories: ['Milano', 'Roma', 'Torino'] },
              },
            ],
          },
          {
            ...
            preprocessing: [{ method: 'standard_scaling', parameters: {} }],
            statistics: {
              mean: 85,
              std: 35,
              min: 25,
            ...
            isRequired: true,
            validationRules: [{ rule: 'range_check', parameters: { min: 20, max: 500 } }],
          },
          {
            name: 'bedrooms',
            type: 'ordinal',
            description: 'Number of bedrooms',
            preprocessing: [{ method: 'ordinal_encoding', parameters: {} }],
            statistics: {
              min: 1,
              max: 6,
              nullCount: 0,
            ...
            isRequired: true,
            validationRules: [{ rule: 'range_check', parameters: { min: 1, max: 10 } }],
          },
          {
            name: 'property_type',
            type: 'categorical',
            description: 'Type of property (apartment, house, villa)',
            preprocessing: [{ method: 'label_encoding', parameters: {} }],
            statistics: {
              nullCount: 0,
              uniqueCount: 4,
            ...
            validationRules: [
              {
                rule: 'in_category_list',
                parameters: { categories: ['Apartment', 'House', 'Villa', 'Loft'] },
              },
            ],
          },
        ],
        ...
        features: [
          {
            ...
            preprocessing: [{ method: 'embedding', parameters: { dim: 64 } }],
            statistics: {
              nullCount: 0,
              uniqueCount: 5000,
            },
            ...
            isRequired: true,
            validationRules: [{ rule: 'not_null', parameters: {} }],
          },
          {
            name: 'property_id',
            type: 'categorical',
          ...
          },
          {
            ...
            validationRules: [
              {
                rule: 'in_category_list',
                parameters: { categories: ['view', 'favorite', 'inquiry', 'contact', 'share'] },
              },
            ],
          },
        ],
        ...
        steps: [
          {
            ...
            configuration: {
              source: 'urbanova_db',
              query:
                'SELECT * FROM real_estate_data WHERE created_at >= CURRENT_DATE - INTERVAL 1 DAY',
          ...
          },
          {
            ...
            outputs: {
              endpoint_url: 'https://api.urbanova.com/ml/predict/property-price-v{{version}}',
            },
          },
        ],
        schedule: {
    ...
    const alerts: MLAlert[] = [
      {
        ...
        description:
          'Significant drift detected in input features for property price prediction model',
        details: {
          drift_score: 0.12,
          threshold: 0.1,
  ...
  private simulateTrainingProgress(trainingRun: TrainingRun) {
    const interval = setInterval(() => {
      ...
      trainingRun.currentEpoch = Math.floor(
        (trainingRun.progress / 100) * (trainingRun.totalEpochs || 100)
      );
      ...
      if (trainingRun.progress >= 100) {
        ...
        trainingRun.duration = Math.floor(
  ...
  // Crea predizione
  createPrediction(modelId: string, input: Record<string, any>, userId?: string): Prediction {
    const model = this.models.get(modelId);
    if (!model) {
      throw new Error(`Model ${modelId} not found`);
    }
  ...
  generateMLStats(): MLStats {
    ...
    const datasetsByType = datasets.reduce(
      (acc, dataset) => {
    ...
    const datasetsByStatus = datasets.reduce(
      (acc, dataset) => {
    ...
    return {
      ...
      predictions: {
        ...
        errorRate:
          (predictions.filter(p => p.status === 'failed').length / predictions.length) * 100,
      },
      training: {
        ...
        averageTrainingTime:
          trainingRuns.filter(tr => tr.duration).reduce((sum, tr) => sum + (tr.duration || 0), 0) /
          Math.max(trainingRuns.filter(tr => tr.duration).length, 1) /
          60, // minutes
        successRate:
          (trainingRuns.filter(tr => tr.status === 'completed').length /
            Math.max(trainingRuns.length, 1)) *
          100,
      },
      experiments: {
        active: experiments.filter(e => e.status === 'running').length,
        completed: experiments.filter(e => e.status === 'completed').length,
        successfulOptimizations: experiments.filter(
          e => e.status === 'completed' && e.results?.winningVariant
        ).length,
      },
      alerts: {
  ...
  searchModels(
    query: string,
    filters?: { type?: ModelType; status?: ModelStatus; owner?: string }
  ): MLModel[] {
