...
export class CloudflareBypass {
  ...
  private async configurePageForBypass(page: Page): Promise<void> {
    // User Agent casuale
    const userAgent = this.bypassConfig.userAgents[
      Math.floor(Math.random() * this.bypassConfig.userAgents.length)
    ];
    await page.setUserAgent(userAgent);

    // Viewport casuale
    const viewport = this.bypassConfig.viewports[
      Math.floor(Math.random() * this.bypassConfig.viewports.length)
    ];
  ...
  private async handleCloudflareChallenge(page: Page): Promise<void> {
    try {
      ...
      const challengeSelector = 'iframe[src*="cloudflare"], .cf-browser-verification, #challenge-form';
  ...
  private async verifyBypassSuccess(page: Page): Promise<boolean> {
    try {
      ...
      const hasChallenge = await page.$('iframe[src*="cloudflare"], .cf-browser-verification, #challenge-form');
  ...
  private async extractCasaItLands(page: Page, criteria: LandSearchCriteria): Promise<ScrapedLand[]> {
  ...
  private async extractIdealistaItLands(page: Page, criteria: LandSearchCriteria): Promise<ScrapedLand[]> {
    const lands: ScrapedLand[] = [];
