...
export class TerritorialIntelligenceService {
   ...
   */
  async analyzeInvestmentOpportunities(zone: string, city: string): Promise<InvestmentOpportunity[]> {
    try {
      ...
      if (trends.length > 0) {
        ...
        if (residentialTrends.length > 0) {
          const avgROI = residentialTrends.reduce((sum, t) => sum + t.analysis.priceChange, 0) / residentialTrends.length;
        ...
        if (commercialTrends.length > 0) {
          const avgROI = commercialTrends.reduce((sum, t) => sum + t.analysis.priceChange, 0) / commercialTrends.length;
      ...
      if (zoneAnalysis.length > 0) {
        ...
        if (analysis.metrics.projectDensity < 2) { // Bassa densità progetti
          opportunities.push({
            id: `opp-${Date.now()}-3`,
            zone,
            city,
            ...
            evidence: {
              dataPoints: [`Densità progetti: ${analysis.metrics.projectDensity}/km²`, 'Bassa concorrenza'],
  ...
  async analyzeInfrastructure(zone: string, city: string): Promise<InfrastructureAnalysis> {
    try {
      ...
      const infrastructureData: InfrastructureAnalysis = {
        ...
        utilities: {
          water: { coverage: 95 + Math.floor(Math.random() * 5), quality: 85 + Math.floor(Math.random() * 10) },
