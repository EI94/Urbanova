...
export class ImprovedWebScraper {
  ...
  private workingUrls = {
    ...
    'subito.it': ['https://www.subito.it/terreni/', 'https://www.subito.it/immobili/terreni/'],
    'kijiji.it': ['https://www.kijiji.it/terreni/', 'https://www.kijiji.it/immobili/terreni/'],
  };
  ...
  private async makeRequest(url: string, domain: string): Promise<any> {
    ...
    try {
      ...
      console.log(
        `❌ Richiesta diretta fallita per ${domain}:`,
    ...
    try {
      ...
      console.log(
        `❌ Richiesta con delay fallita per ${domain}:`,
        error instanceof Error ? error.message : 'Errore sconosciuto'
      );
    }
  ...
  private extractPrice($el: any): number | null {
    const priceText = $el
      .find('[class*="price"], [class*="Price"], .price, .Price, [data-price]')
      .first()
      .text();
    if (priceText) {
  ...
  private extractArea($el: any): number | null {
    const areaText = $el
      .find('[class*="area"], [class*="Area"], .area, .Area, [data-area]')
      .first()
      .text();
    if (areaText) {
  ...
  private extractTitle($el: any): string | null {
    const titleText = $el
      .find('h1, h2, h3, [class*="title"], [class*="Title"], .title, .Title')
      .first()
      .text();
